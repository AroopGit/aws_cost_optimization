    ...
    return (
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <button
                ...
                disabled={loading}
                className="w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 transition mb-6"
            >
                    ...
                    <div className="h-64 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={results}>
                                ...
                                />
                                <Legend />
                                <Line type="monotone" dataKey="candidates.price_optimal.price" name="Optimal" stroke="#3b82f6" strokeWidth={2} dot={false} />
                                ...
                                {results.slice(0, 10).map((row, idx) => (
                                    <React.Fragment key={idx}>
                                        <tr className="hover:bg-slate-50 transition cursor-pointer" onClick={() => toggleRow(idx)}>
                                            ...
                                            <td className="px-3 py-2 text-right font-medium text-blue-600">₹{row.candidates.price_optimal.price}</td>
                                                    ...
                                                    <div className="space-y-4">
                                                                ...
                                                                {Object.entries(row.candidates).map(([key, data]) => (
                                                                    <div key={key} className="bg-white p-2 rounded border border-slate-100 shadow-sm">
                                                                        ...
                                                                        <div className="flex justify-between text-[10px] mb-1">
                                                                            ...
                                                                            <span className="font-medium text-blue-600">₹{data.price}</span>
                                                                        </div>
                                                                        <div className="flex justify-between text-[10px] mb-1">
                                                        ...
                                                        <div className="bg-blue-50 p-2 rounded border border-blue-100">
                                                            <div className="flex justify-between items-center">
                                                                <span className="text-xs font-semibold text-blue-800">Final Selection</span>
                                                                <span className="text-sm font-bold text-blue-700">₹{row.selection.price_recommended}</span>
                                                            </div>
                                                            <div className="text-[10px] text-blue-600 mt-1">{row.selection.approval} ({row.selection.change_pct}%)</div>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
